...
def main():
    with beam.Pipeline('DirectRunner') as p:
        (p
         ...
         | beam.GroupBy('date').aggregate_field('transaction_amount', sum, 'total_amount')
        #  | beam.io.WriteToText('output/results.jsonl.gz', num_shards=1, compression_type=beam.io.filesystem.CompressionTypes.GZIP, shard_name_template='')
         | beam.Map(to_json)
         | beam.io.WriteToText('output/results.jsonl', shard_name_template='')
        )
        p.run()
